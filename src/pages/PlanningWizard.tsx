...
export default function PlanningWizard() {
  ...
  return (
    <div className="min-h-dvh bg-[var(--color-bg)] px-4 py-5">
      <div className="mx-auto max-w-lg">
        <div className="mb-5 flex items-center justify-between">
          <button onClick={() => navigate('/')} className="text-base text-[var(--color-text-muted)]">
            ← Cancel
          </button>
          <h1 className="text-xl font-bold text-[var(--color-primary-light)]">Plan My Week</h1>
          <div className="w-16" />
        </div>
          ...
          <div>
            <div className="mb-5 flex gap-2">
              <input
                ...
                className="flex-1 rounded-xl border border-[var(--color-surface-light)] bg-[var(--color-surface)] p-3.5 text-base text-[var(--color-text)] placeholder:text-[var(--color-text-muted)] focus:border-[var(--color-primary)] focus:outline-none"
              />
              <button
                onClick={addTask}
                className="rounded-xl bg-[var(--color-primary)] px-5 text-base font-semibold text-white"
              >
                Add
              </button>
            </div>
            ...
            {incompleteTasks.length > 0 && (
              <div className="mb-5 rounded-xl border border-[var(--color-accent)]/30 bg-[var(--color-surface)] p-4">
            ...
            <div className="flex flex-col gap-2.5">
              {tasks.map(task => (
                <div key={task.id} className="flex items-center gap-3 rounded-xl bg-[var(--color-surface)] p-3.5">
                  <span className="text-base">
                    {CATEGORIES.find(c => c.value === task.category)?.icon} {task.title}
                  </span>
                  <button
                    onClick={() => removeTask(task.id)}
                    className="ml-auto text-base text-[var(--color-danger)]"
                  >
                    ✕
                  </button>
                </div>
            ...
            {tasks.length === 0 && (
              <p className="py-12 text-center text-base text-[var(--color-text-muted)]">Add your tasks for the week above</p>
            )}
          </div>
        )}
        ...
        {step === 2 && (
          <div>
            <div className="mb-5 flex gap-1">
              {tasksPerDay.map(({ day, count }) => (
                <div key={day} className="flex-1 text-center">
                  <div className="text-xs text-[var(--color-text-muted)]">{formatDayShort(day)}</div>
            ...
            <div className="flex flex-col gap-3">
              {tasks.map(task => (
                <div key={task.id} className="rounded-xl bg-[var(--color-surface)] p-4">
              ...
              <div key={task.id} className="rounded-xl bg-[var(--color-surface)] p-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex gap-1.5">
                    {(['high', 'medium', 'low'] as Priority[]).map(p => (
                      <button
                        ...
                        className={`rounded-full px-3.5 py-2 text-sm capitalize transition-colors ${
                          task.priority === p
                  ...
                  <select
                    ...
                    className="rounded-lg bg-[var(--color-surface-light)] px-3 py-2 text-sm text-[var(--color-text)]"
                  >
        ...
        {step === 4 && (
          <div>
              ...
              return (
                <div key={day} className="mb-5">
                  <div className="mb-2 flex items-center justify-between">
                    <h3 className="text-base font-bold">{formatDayFull(day)}</h3>
                  ...
                  <div className="flex flex-col gap-2">
                    {dayTasks.map(task => (
                      <div key={task.id} className="flex items-center gap-3 rounded-xl bg-[var(--color-surface)] p-3.5 text-base">
            ...
            {tasks.filter(t => t.days.length === 0).length > 0 && (
              <div className="mb-5 rounded-xl border border-[var(--color-accent)] bg-[var(--color-surface)] p-4">
        ...
        <div className="mt-8 flex justify-between">
          <button
            ...
            className="rounded-xl bg-[var(--color-surface)] px-6 py-3 text-base text-[var(--color-text-muted)]"
          >
            ← {step === 1 ? 'Cancel' : 'Back'}
          </button>
          {step < 4 ? (
            <button
              ...
              className="rounded-xl bg-[var(--color-primary)] px-6 py-3 text-base font-semibold text-white disabled:opacity-50"
            >
              Next →
            </button>
          ) : (
            <div className="flex gap-2">
              <button
                ...
                className="rounded-xl border border-[var(--color-accent)] px-4 py-3 text-sm text-[var(--color-accent)]"
              >
                📅 Export
              </button>
              <button
                onClick={finalizePlan}
                className="rounded-xl bg-[var(--color-success)] px-6 py-3 text-base font-semibold text-[var(--color-bg)]"
              >
                Lock In My Plan ✓
              </button>
            </div>
